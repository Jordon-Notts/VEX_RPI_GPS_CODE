...
def compute_camera_global_pose(rvec_camera_to_aruco, 
    ...
    """
    Compute the camera's global pose (rvec, tvec) given:
      - The camera-to-marker (ArUco) pose (rvec_camera_to_aruco, tvec_camera_to_aruco),
      - The marker's pose in the global frame (rvec_aruco_in_global, tvec_aruco_in_global).
      
    Returns:
        tuple (rvec_camera_in_global, tvec_camera_in_global)
    """

    # Ensure float32
    rvec_camera_to_aruco = np.asarray(rvec_camera_to_aruco, dtype=np.float32)
    tvec_camera_to_aruco = np.asarray(tvec_camera_to_aruco, dtype=np.float32)
    rvec_aruco_in_global = np.asarray(rvec_aruco_in_global, dtype=np.float32)
    tvec_aruco_in_global = np.asarray(tvec_aruco_in_global, dtype=np.float32)

    # Convert rotation vectors to rotation matrices
    R_camera_to_aruco, _ = cv2.Rodrigues(rvec_camera_to_aruco)
    R_aruco_in_global, _ = cv2.Rodrigues(rvec_aruco_in_global)

    # Compute the camera's rotation in global coordinates:
    #   R_camera_in_global = R_aruco_in_global * R_camera_to_aruco
    R_camera_in_global = np.dot(R_aruco_in_global, R_camera_to_aruco)
    rvec_camera_in_global, _ = cv2.Rodrigues(R_camera_in_global)

    # Compute the camera’s translation in global coordinates:
    #   t_camera_in_global = t_aruco_in_global + R_aruco_in_global * t_camera_to_aruco
    t_camera_in_global = tvec_aruco_in_global + np.dot(R_aruco_in_global, tvec_camera_to_aruco)
